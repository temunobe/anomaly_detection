...
def load_and_prepare_data(
    ...
    if kfold is not None and kfold > 1:
        ...
        for train_idx, test_idx in skf.split(df.index, df['Label'].values):
    ...
    # Only normalize columns that are numeric in selected_features
    numeric_cols = []
    ...
    # The caller may set config flags via kwargs in the wrapper; default behaviors are conservative.
    # We'll look for these flags in the model argument if it's a dict-like config, else defaults.
    # NOTE: the run scripts will pass normalize_numeric and oversample_minority via kwargs.
    
    # apply optional normalization and augmentation/oversampling if provided via kwargs
        # loader expects explicit flags passed from the run script; defaults are above
    ...
    try:
        oversample_minority = getattr(model, 'oversample_minority', False)
    except Exception:
    ...
    # Apply numeric jitter augmentation to training split only (after normalization if applied)
